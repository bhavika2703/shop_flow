...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  Future<void> _onFetchProducts(FetchProducts event, Emitter<ProductState> emit) async {
    ...
    res.fold((failure) => emit(ProductLoadFailure(message: failure.message)), (newItems) {
      final all = [...items, ...newItems];
      emit(ProductLoadSuccess(products: all, hasReachedMax: newItems.length < pageSize));
    });
  }
  ...
  Future<void> _onRefreshProducts(RefreshProducts event, Emitter<ProductState> emit) async {
    ...
    res.fold((failure) => emit(ProductLoadFailure(message: failure.message)), (items) {
      emit(ProductLoadSuccess(products: items, hasReachedMax: items.length < pageSize));
    });
  }
}
